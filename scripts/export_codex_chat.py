...
def load_threads(codex_home: Path) -> list[ThreadRecord]:
    ...
    if threads:
        return threads
    return load_threads_from_rollouts(codex_home)
...
def normalize_message(message_index: int, record: dict[str, Any]) -> dict[str, Any] | None:
...
def parse_rollout(rollout_path: Path, include_tools: bool) -> dict[str, Any]:
    ...
    with rollout_path.open("r", encoding="utf-8") as handle:
        for line in handle:
            ...
            if record.get("type") == "session_meta":
                ...
                continue
            item = normalize_message(message_index, record)
            if not item:
                continue
...
def select_threads(
    ...
    if thread_id:
        return [thread for thread in threads if thread.id == thread_id]
    if contains:
    ...
    if export_all:
        return threads
    raise SystemExit("Choose one of --id, --contains, --latest, or --all.")
...
def command_list(args: argparse.Namespace) -> int:
    ...
    for thread in threads:
        ...
        if len(title) > 120:
            title = title[:117] + "..."
        print(f"{thread.id}\t{updated_at}\t{title}")
    return 0
...
def resolve_output_path(
    ...
    if multi or raw.is_dir() or output.endswith(os.sep):
        ...
        filename = f"{thread.id}-{slugify(thread.title)[:80]}.{suffix}"
        return raw / filename
    return raw
...
def command_export(args: argparse.Namespace) -> int:
    threads = load_threads(args.codex_home)
    selected = select_threads(threads, args.id, args.latest, args.contains, args.all)
    if not selected:
    ...
    for thread in selected:
        transcript = parse_rollout(Path(thread.rollout_path), include_tools=args.include_tools)
        output_path = resolve_output_path(args.output, args.format, thread, multi)
        if args.format == "markdown":
            rendered = render_markdown(thread, transcript)
        else:
            rendered = json.dumps(
                build_export_payload(thread, transcript),
                ensure_ascii=False,
                indent=2,
            ) + "\n"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    selector.add_argument("--all", action="store_true", help="Export all threads")
    ...
    export_parser.add_argument(
        "--output",
        help="Output file path or directory. Prints to stdout when omitted.",
    )
    export_parser.add_argument(
        "--include-tools",
        action="store_true",
        help="Include tool call and tool result records in the export.",
    )
